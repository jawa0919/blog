...
download_and_install() {
  ...
  if [ -z "${PNPM_VERSION}" ]; then
    ...
    # version_json="$(download "https://registry.npmmirror.com/@pnpm/exe")" || abort "Download Error!"
    version_json="$(download "https://mirrors.huaweicloud.com/repository/npm/@pnpm/exe")" || abort "Download Error!"
  ...
  printf 'Downloading pnpm from %s' "${archive_url}"
